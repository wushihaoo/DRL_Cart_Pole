...
class DDQN:
    ...
    def update(self):
        ...
        max_q_eval_action = self.eval_net(sample_next_state).argmax(dim=1).detach()
        q_next_state = self.target_net(sample_next_state).gather(1, max_q_eval_action.unsqueeze(1)).detach()
